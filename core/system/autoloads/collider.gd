...
func try_merge(target: Vector2, a: Rect2i, b: Rect2i, player_size: float = 8) -> Variant:
	...
	for i: int in [0, 2]:
		...
		if nearest_a.x + nearest_b.x == i and a.position.x == b.position.x and nearest_a.y != nearest_b.y:
			if nearest_a.y == 1 and nearest_b.y == 0:
				if -(a.end.y - b.position.y) < player_size:
					return a.merge(b)
			else:
				if (a.position.y - b.end.y) < player_size:
					return a.merge(b)
		# Case: Walls are above or below the player.
			...
			if nearest_a.x == 1 and nearest_b.x == 0:
				if -(a.end.x - b.position.x) < player_size:
					return a.merge(b)
			else:
				if (a.position.x - b.end.x) < player_size:
					return a.merge(b)
